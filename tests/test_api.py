...
def _client_with_mocks(monkeypatch, tmp_path):
    import app.main as main_mod
    from app import config

    monkeypatch.setattr(config, "FACE_DB_PATH", tmp_path / "face_db.pkl")
    ...
    monkeypatch.setattr(main_mod, "_svc", lambda: FakeSvc())
    return TestClient(main_mod.app)


def test_health(monkeypatch, tmp_path):
    c = _client_with_mocks(monkeypatch, tmp_path)
    r = c.get("/api/health")
    assert r.status_code == 200
    assert r.json()["status"] == "ok"


def test_detect_objects(monkeypatch, tmp_path):
    c = _client_with_mocks(monkeypatch, tmp_path)
...
def test_empty_upload_rejected(monkeypatch, tmp_path):
    c = _client_with_mocks(monkeypatch, tmp_path)
